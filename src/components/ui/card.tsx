...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-(--card-spacing) rounded-xl border-2 border-foreground/70 bg-card py-(--card-spacing) text-sm text-card-foreground shadow-[3px_3px_0_0_#401f32] transition-all duration-150 hover:shadow-none active:shadow-[1px_1px_0_0_#401f32] [--card-spacing:--spacing(4)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(3)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-xl *:[img:last-child]:rounded-b-xl",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-xl border-t-2 border-foreground/15 bg-muted/50 p-(--card-spacing)",
        className
      )}
      {...props}
    />
